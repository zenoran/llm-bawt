...
def parse_arguments(config_obj: "Config") -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        metavar="TYPE",
        help="Add models: 'ollama' (refresh from server), 'openai' (query API), 'gguf' (add from HuggingFace repo), 'vllm' (add vLLM model from HuggingFace), 'openclaw' (bind OpenClaw session/channel)"
    )
    parser.add_argument(
        "--delete-model",
        type=str,
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Walk through client .env settings interactively (pre-filled with current values)."
    )
