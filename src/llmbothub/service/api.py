"""
FastAPI-based background service for llmbothub.
...
Run with: python -m llmbothub.service.server
Or: uvicorn llmbothub.service.server:app --host 0.0.0.0 --port 8642
"""
...
class BackgroundService:
    ...
    def __init__(self, config: Config):
        ...
        # Cached LLMBotHub instances keyed by (model_alias, bot_id, user_id)
        self._llmbothub_cache: dict[tuple[str, str, str], Any] = {}
    ...
    def _on_model_unloaded(self, model_alias: str):
        ...
        # Clear all LLMBotHub instances that use this model
        keys_to_remove = [
            key for key in self._llmbothub_cache
            if key[0] == model_alias
        ]
        for key in keys_to_remove:
            del self._llmbothub_cache[key]
    ...
    def _get_llmbothub(self, model_alias: str, bot_id: str, user_id: str, local_mode: bool = False):
        """Get or create an LLMBotHub instance with caching.
        ...
        """
        from .core import ServiceLLMBotHub
        ...
        if cache_key in self._llmbothub_cache:
            log.cache_hit("llmbothub", f"{model_alias}/{bot_id}/{user_id}")
            return self._llmbothub_cache[cache_key]
        
        log.cache_miss("llmbothub", f"{model_alias}/{bot_id}/{user_id}")
        ...
        try:
            # Create a copy of config for each ServiceLLMBotHub instance
            ...
            self._llmbothub_cache[cache_key] = llmbothub
            
            # Also cache the client for future reuse by extraction tasks
            if model_alias not in self._client_cache:
                self._client_cache[model_alias] = llmbothub.client
            # Note: BaseLLMBotHub.__init__ already registers with lifecycle manager
    ...
    def get_client(self, model_alias: str):
        ...
        so we cache the client independently from LLMBotHub instances.
        """
        if model_alias in self._client_cache:
        ...
        # Check if we already have an LLMBotHub instance with this model
        # and can reuse its client
        for (cached_model, _, _), llmbothub in self._llmbothub_cache.items():
            if cached_model == model_alias:
                log.debug(f"Reusing client from existing LLMBotHub instance for '{model_alias}'")
                self._client_cache[model_alias] = llmbothub.client
                return llmbothub.client
        ...
        llmbothub = self._get_llmbothub(model_alias, "spark", "system", local_mode=True)
        self._client_cache[model_alias] = llmbothub.client
        return llmbothub.client
    ...
    async def chat_completion(
        ...
        1. Uses llmbothub's internal history + memory for context
        ...
        # Get cached LLMBotHub instance
        llmbothub = self._get_llmbothub(model_alias, bot_id, user_id, local_mode)
        ...
        try:
            ...
            def _do_query():
                ...
                if self.config.DEBUG_TURN_LOG or os.environ.get("LLMBOTHUB_DEBUG_TURN_LOG"):
                ...
                llmbothub.finalize_response(user_prompt, response, tool_context)
    ...
    async def chat_completion_stream(
        ...
        Uses llmbothub's internal history + memory for context.
        ...
        def _stream_to_queue():
            ...
            try:
                ...
                # Use llmbothub.prepare_messages_for_query to get full context
                # (history from DB + memory + system prompt)
                messages = llmbothub.prepare_messages_for_query(user_prompt)
                ...
                if llmbothub.bot.uses_tools and llmbothub.memory:
                    # Check if client supports native streaming with tools (OpenAI)
                    use_native_streaming = (
                        llmbothub.client.supports_native_tools()
                        and llmbothub.tool_format in ("native", "NATIVE_OPENAI")
                        and hasattr(llmbothub.client, "stream_with_tools")
                    )
                    ...
                    if use_native_streaming:
                        ...
                        tool_definitions = llmbothub._get_tool_definitions()
                        handler = get_format_handler(llmbothub.tool_format)
                        tools_schema = handler.get_tools_schema(tool_definitions)

                        executor = ToolExecutor(
                            memory_client=llmbothub.memory,
                            profile_manager=llmbothub.profile_manager,
                            search_client=llmbothub.search_client,
                            model_lifecycle=llmbothub.model_lifecycle,
                            config=llmbothub.config,
                            user_id=llmbothub.user_id,
                            bot_id=llmbothub.bot_id,
                        )
                        ...
                        def native_stream_with_tool_loop():
                            ...
                            for iteration in range(max_iterations):
                                for item in llmbothub.client.stream_with_tools(
                                    current_msgs,
                        ...
                        log.debug(f"Using stream_with_tools for tool format: {llmbothub.tool_format}")
                        
                        adapter = getattr(llmbothub, 'adapter', None)
                        if adapter:
                            log.debug(f"Passing adapter '{adapter.name}' to stream_with_tools")
                        else:
                            log.warning("No adapter found on llmbothub instance")

                        def stream_fn(msgs, stop_sequences=None):
                            return llmbothub.client.stream_raw(msgs, stop=stop_sequences)
                        ...
                        stream_iter = stream_with_tools(
                            ...
                            user_id=llmbothub.user_id,
                            bot_id=llmbothub.bot_id,
                            tool_format=llmbothub.tool_format,
                            adapter=adapter,
                        )
                else:
                    stream_iter = llmbothub.client.stream_raw(messages)
                ...
                if full_response_holder[0] and not cancel_event.is_set():
                    ...
                    llmbothub.finalize_response(user_prompt, full_response_holder[0], tool_context_holder[0])
    ...
    async def _process_extraction(self, task: Task) -> dict:
        ...
        if not model_to_use:
            ...
            if model_to_use in self._client_cache:
                ...
                # Check if any LLMBotHub instance has this model loaded
                for (cached_model, _, _), llmbothub in self._llmbothub_cache.items():
                    if cached_model == model_to_use:
                        extraction_client = llmbothub.client
                        self._client_cache[model_to_use] = extraction_client
                        log.debug(f"Reusing client from LLMBotHub instance for extraction: {model_to_use}")
                        break
    ...
    async def _process_profile_maintenance(self, task: Task) -> dict:
        ...
        def first_local_cached() -> str | None:
            for alias in self._client_cache.keys():
                if not is_openai_model(alias):
                    return alias
            for (alias, _, _), _llmbothub in self._llmbothub_cache.items():
                if not is_openai_model(alias):
                    return alias
            return None
        ...
        if not llm_client:
            ...
            try:
                # Create an LLMBotHub instance which will load the model
                llmbothub = self._get_llmbothub(
...
try:
    ...
    class NextcloudProvisionRequest(BaseModel):
        ...
        bot_id: str = Field(description="llmbothub bot ID (e.g., nova, monika)")
    ...
    async def provision_nextcloud_talk(request: NextcloudProvisionRequest):
        """Provision a Nextcloud Talk room and bot for an llmbothub bot."""
    ...
    async def reload_nextcloud_bots():
        ...
        log.info(f"🔄 Reloaded {len(bots)} Nextcloud bots: {[b.llmbothub_bot for b in bots]}")
        return {
            "status": "reloaded",
            "bots_count": len(bots),
            "bots": [b.llmbothub_bot for b in bots],
        }
    ...
    async def chat_completions(request: ChatCompletionRequest):
        ...
        Supports all standard OpenAI parameters plus llmbothub extensions:
    ...
    async def clear_history(
        ...
        try:
            ...
            llmbothub = service._get_llmbothub(model_alias, effective_bot_id, service.config.DEFAULT_USER)
            ...
            if cache_key in service._llmbothub_cache:
                del service._llmbothub_cache[cache_key]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="llmbothub background service")
