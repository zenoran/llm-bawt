...
DEFAULT_SYSTEM_MESSAGE = """You are Nova, a personal AI assistant running locally via llmbothub.
...
def get_default_config_dir() -> Path:
    ...
    default_config_dir = Path(xdg_config_home) / "llmbothub"
    default_config_dir.mkdir(parents=True, exist_ok=True)
    return default_config_dir


def get_default_models_yaml_path() -> Path:
    env_path = os.environ.get("LLMBOTHUB_MODELS_CONFIG_PATH")
    if env_path:
...
def _env_file_has_llmbothub_key(path: Path) -> bool:
    try:
...
def get_dotenv_path() -> Path:
    env_override = os.environ.get("LLMBOTHUB_ENV_FILE") or os.environ.get("LLMBOTHUB_DOTENV_PATH")
    if env_override:
        return Path(env_override).expanduser().resolve()

    cwd_env = Path.cwd() / ".env"
    if cwd_env.is_file() and _env_file_has_llmbothub_key(cwd_env):
        return cwd_env

    env_models = os.environ.get("LLMBOTHUB_MODELS_CONFIG_PATH")
    if env_models:
...
class Config(BaseSettings):
    HISTORY_FILE: str = Field(default=os.path.expanduser("~/.cache/llmbothub/chat-history"))
    HISTORY_DURATION: int = Field(default=60 * 30)  # 30 minutes in seconds
    OLLAMA_URL: str = Field(default="http://localhost:11434")
    MODEL_CACHE_DIR: str = Field(default=os.path.expanduser("~/.cache/llmbothub/models"), description="Directory to cache downloaded GGUF models")
    ...
    DEFAULT_USER: Optional[str] = Field(
        default=None,
        description="User profile to use. Set via LLMBOTHUB_DEFAULT_USER env var.",
    )

    # --- Memory Settings --- #
    MEMORY_N_RESULTS: int = Field(default=10, description="Number of relevant memories to retrieve during search (Set via LLMBOTHUB_MEMORY_N_RESULTS)")
    MEMORY_PROTECTED_RECENT_TURNS: int = Field(default=3, description="Number of recent conversation turns to always include regardless of token limits (Set via LLMBOTHUB_MEMORY_PROTECTED_RECENT_TURNS)")
    MEMORY_MIN_RELEVANCE: float = Field(default=0.01, description="Minimum relevance score (0.0-1.0) for memories to be included (Set via LLMBOTHUB_MEMORY_MIN_RELEVANCE)")
    MEMORY_MAX_TOKEN_PERCENT: int = Field(default=30, description="Maximum percentage of context window to use for memories (Set via LLMBOTHUB_MEMORY_MAX_TOKEN_PERCENT)")
    MEMORY_DEDUP_SIMILARITY: float = Field(default=0.85, description="Similarity threshold (0.0-1.0) for fuzzy deduplication of memories against history (Set via LLMBOTHUB_MEMORY_DEDUP_SIMILARITY)")
    ...
    MEMORY_SERVER_URL: Optional[str] = Field(
        default=None,
        description="Optional MCP memory server base URL (Set via LLMBOTHUB_MEMORY_SERVER_URL)",
    )

    # --- PostgreSQL Memory Backend Settings --- #
    POSTGRES_HOST: str = Field(default="localhost", description="PostgreSQL server hostname (Set via LLMBOTHUB_POSTGRES_HOST)")
    POSTGRES_PORT: int = Field(default=5432, description="PostgreSQL server port (Set via LLMBOTHUB_POSTGRES_PORT)")
    POSTGRES_USER: str = Field(default="llmbothub", description="PostgreSQL username (Set via LLMBOTHUB_POSTGRES_USER)")
    POSTGRES_PASSWORD: str = Field(default="", description="PostgreSQL password (Set via LLMBOTHUB_POSTGRES_PASSWORD)")
    POSTGRES_DATABASE: str = Field(default="llmbothub", description="PostgreSQL database name (Set via LLMBOTHUB_POSTGRES_DATABASE)")
