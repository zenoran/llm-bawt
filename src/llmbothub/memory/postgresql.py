"""PostgreSQL memory backend for llmbothub with pgvector support.
...
class PostgreSQLMemoryBackend(MemoryBackend):
    ...
    Configuration (via environment variables or .env):
        LLMBOTHUB_POSTGRES_HOST: Database host (default: localhost)
        LLMBOTHUB_POSTGRES_PORT: Database port (default: 5432)
        LLMBOTHUB_POSTGRES_USER: Database user
        LLMBOTHUB_POSTGRES_PASSWORD: Database password
        LLMBOTHUB_POSTGRES_DATABASE: Database name (default: llmbothub)
    """
    ...
    def __init__(self, config: Any, bot_id: str = "nova", embedding_dim: int | None = None):
        ...
        user = getattr(config, 'POSTGRES_USER', 'llmbothub')
        password = getattr(config, 'POSTGRES_PASSWORD', '')
        database = getattr(config, 'POSTGRES_DATABASE', 'llmbothub')
