...
# Provisioner configuration from environment (prefer LLMBOTHUB_ prefix)
PROVISIONER_URL = os.getenv('LLMBOTHUB_TALK_PROVISIONER_URL') or os.getenv('TALK_PROVISIONER_URL', 'http://localhost:8790')
PROVISIONER_TOKEN = os.getenv('LLMBOTHUB_TALK_PROVISIONER_TOKEN') or os.getenv('TALK_PROVISIONER_TOKEN')
...
class ProvisionerClient:
    ...
    def __init__(
        ...
        if not token:
            raise ValueError(
                "LLMBOTHUB_TALK_PROVISIONER_TOKEN (or TALK_PROVISIONER_TOKEN) environment variable is required"
            )
