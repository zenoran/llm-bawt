...
User config (~/.config/llmbothub/bots.yaml) takes priority over repo config.
This module uses the centralized bot config loader from llmbothub.bots.
"""

from dataclasses import dataclass
from typing import Optional

from llmbothub.bots import (
...
class NextcloudBot:
    ...
    llmbothub_bot: str  # The bot ID in bots.yaml
    ...
    @property
    def name(self) -> str:
        """Bot name is the llmbothub bot ID."""
        return self.llmbothub_bot
...
class NextcloudBotConfig:
    ...
    - User bots.yaml (~/.config/llmbothub/bots.yaml) (overrides)
    ...
    def __init__(self, bots_yaml_path=None):
        ...
        Args:
            bots_yaml_path: Ignored - kept for backwards compatibility.
                           Config loading is now centralized in llmbothub.bots.
        """
    ...
    def add_bot(
        ...
        Saves to ~/.config/llmbothub/bots.yaml (user config, not repo).
        """
        nc_data = {
        ...
        save_user_bot_config(llmbothub_bot, 'nextcloud', nc_data)
    ...
    def remove_bot(self, llmbothub_bot: str) -> bool:
        """Remove Nextcloud config from a bot.
        
        Removes from user config only (doesn't touch repo).
        """
        return remove_user_bot_section(llmbothub_bot, 'nextcloud')
    ...
    def get_bot(self, llmbothub_bot: str) -> Optional[NextcloudBot]:
        """Get bot config by llmbothub bot ID."""
        return self.bots.get(llmbothub_bot)
