...
class EventRenderer:
    ...
    def __init__(self, console: Console, verbose: bool = False, debug: bool = False) -> None:
        ...
        self.debug = debug
        self._logger = logging.getLogger("llmbothub")
...
class LogConfig:
    ...
    def configure(cls, *, verbose: bool = False, debug: bool = False) -> None:
        ...
        log_prefix = os.getenv("LLMBOTHUB_LOG_PREFIX", "").strip()
        log_dir = os.getenv("LLMBOTHUB_LOG_DIR", ".logs")
        console_level = logging.INFO if (verbose or debug) else logging.WARNING
        root_level = logging.DEBUG if debug else console_level
        log_file_name = f"{log_prefix}.debug.log" if log_prefix else "llmbothub.debug.log"
        ...
        if debug:
            ...
            logging.getLogger("llmbothub").setLevel(logging.INFO)
            return

        if verbose:
            # Verbose mode: Show INFO from llmbothub with Rich formatting
            handler = RichHandler(
                console=console,
                show_path=False,
                show_time=False,
