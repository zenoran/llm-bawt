"""
Manages AI model configurations for the llmbothub tool.

This module provides functionality to list, update, and delete model aliases
stored in the models configuration file (typically `~/.config/llmbothub/models.yaml`).
...
from llmbothub.utils.config import (
    PROVIDER_GGUF,
    PROVIDER_HF,
    PROVIDER_OLLAMA,
    PROVIDER_OPENAI,
...
def is_service_mode_enabled(config: Config) -> bool:
    ...
    Returns:
        True if USE_SERVICE is enabled via LLMBOTHUB_USE_SERVICE env var or config.USE_SERVICE
    """
...
class ModelManager:
    ...
    def list_available_models(self):
        ...
        if use_service:
            try:
                from llmbothub.service.client import get_service_client
    ...
    def _get_dependency_note(model_type: str) -> str:
        ...
        if model_type == PROVIDER_HF:
            return " (Requires: [dim]llmbothub[huggingface][/dim])"
        if model_type == PROVIDER_GGUF:
